...
-ui |--uionly              [if specified the insaller will only install and configure wireguard-ui based on the specified config file]
-w  |--config|--configpath [the path where WireGuard config will be created (/etc/wireguard)]
-pki|--privatekey          [specifies an existing private instead of generating a new one.]
...
-c  |--cidr                [the server interface address, the wg server will be assigned the first IP of the range, i.e: 10.8.0.1 unless --serverip is specified (10.8.0.0/26)]
-ip |--serverip            [The wg server will be assigned the first IP of the CIDR range, --server ip overrides this. (null)]
...
-lo |--local               [if specified, the script will use tge local wireguard-ui build included with this script]
...
-l  |--localonly           [ <1|0> if set to 1 the web ui can be accessed only from localhost (default: 1)]
